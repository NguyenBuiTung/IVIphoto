...
namespace TakeimgIVI.Camera
{
    /// <summary>
    /// MyCamera
    /// </summary>
    public class CameraCIC2
    {
        //Function & variables
        [DllImport("kernel32.dll", EntryPoint = "CopyMemory", SetLastError = false)]
        public static extern void CopyMemory(IntPtr dest, IntPtr src, uint count);
        public static MV_CC_DEVICE_INFO_LIST DeviceList = new MV_CC_DEVICE_INFO_LIST();
        public static IntPtr DeviceCIC = IntPtr.Zero;

        public static PixelFormat bmpPF;
        public static Bitmap bmp;
        public static Bitmap TrueBitmap;
        public static IntPtr ConvertBuffer = IntPtr.Zero;
        public static IntPtr m_BufForDriver = IntPtr.Zero;
        public static IntPtr PictureBoxHandle;
        private static Object BufForDriverLock = new Object();
        private static UInt32 m_nBufSizeForDriver = 0;
        public static uint BufferLength = 0;
        public static MV_FRAME_OUT_INFO_EX FrameInfo = new MV_FRAME_OUT_INFO_EX();
        public static bool GrabbingCIC;
        public static bool WaitingImage;
        public static bool ErrorTake = false;
        public static Thread ReceiveProcess;
        public static bool ManualTrigger = false;
        public string IP;
        public bool IsConnecting;
        public MV_CC_DEVICE_INFO DeviceInfo = new MV_CC_DEVICE_INFO();
        //lấy dải dữ liệu từ số
        public string GetDeviceIp(uint ip)
        {
            var r = ip;
            string u = (r >> 24 & 255) + "." + (r >> 16 & 255) + "." + (r >> 8 & 255) + "." + (r & 255);
            return u;
        }
		// kiểm tra kết nối(theo mẫu)
		public void CheckConnection()
        {
            if (!Status.PingHost(Constants.Camera_IP2))
            {
        ...
        public void GetDeviceInfo()
        {
            ...
            {
                MV_CC_DEVICE_INFO device = (MV_CC_DEVICE_INFO)Marshal.PtrToStructure(DeviceList.pDeviceInfo[i], typeof(MV_CC_DEVICE_INFO));
                if (device.nTLayerType == MV_GIGE_DEVICE)
                {
                    ...
                    if (GetDeviceIp(gigeInfo.nCurrentIp) == Constants.Camera_IP2)
                        {
                            Constants.Camera_IP2 = GetDeviceIp(gigeInfo.nCurrentIp);
                            INI.WriteIni(Constants.Common, "Camera", "IP2", Constants.Camera_IP2);
                            DeviceInfo = device;
                            IP = Constants.Camera_IP2;
                            DeviceCIC = DeviceList.pDeviceInfo[i];
                            HaveConnection = true;
                            break;
                        }
                    }
            ...
            if (!HaveConnection)
            {
                DeviceCIC = IntPtr.Zero;
            }
            // ch:选择第一项 | en:Select the first item
            if (DeviceCIC != IntPtr.Zero)
            {
                DelegateToUI.PushToListBox("Get Device Camera 2 successfully!");
            }
            else
            {
                DelegateToUI.PushToListBox("Fail to get Camera 2 Device", LogTitle.Error);
            }
        }
		// mở thiết bị(theo mẫu)
		public bool OpenDevice()
        {
            MV_CC_DEVICE_INFO device = (MV_CC_DEVICE_INFO)Marshal.PtrToStructure(DeviceCIC, typeof(MV_CC_DEVICE_INFO));

            int Status = MV_CC_CreateDevice_NET(ref device);
            if (MV_OK != Status)
            {
                DelegateToUI.PushToListBox("Create device Camera 2 fail!", LogTitle.Error);
                return false;
            }

            Status = MV_CC_OpenDevice_NET();
            if (MV_OK != Status)
            {
                MV_CC_DestroyDevice_NET();
                DelegateToUI.PushToListBox("Device open Camera 2 fail!", LogTitle.Error);
                return false;
            }

            int nPacketSize = MV_CC_GetOptimalPacketSize_NET();
            if (nPacketSize > 0)
            {
                Status = MV_CC_SetIntValueEx_NET("GevSCPSPacketSize", nPacketSize);
                if (Status != MV_OK)
                {
                    DelegateToUI.PushToListBox("Set Packet Size Camera 2 failed!", LogTitle.Error);
                }
            }
            else
            {
                DelegateToUI.PushToListBox("Get Packet Size Camera 2 failed!", LogTitle.Error);
            }

            MV_CC_SetEnumValue_NET("AcquisitionMode", (uint)MV_CAM_ACQUISITION_MODE.MV_ACQ_MODE_CONTINUOUS);
            MV_CC_SetEnumValue_NET("TriggerMode", (uint)MV_CAM_TRIGGER_MODE.MV_TRIGGER_MODE_OFF);
            MVCC_FLOATVALUE stParam = new MVCC_FLOATVALUE();
            int nRet = MV_CC_GetFloatValue_NET("ExposureTime", ref stParam);
            nRet = MV_CC_GetFloatValue_NET("Gain", ref stParam);
            nRet = MV_CC_GetFloatValue_NET("ResultingFrameRate", ref stParam);
            return true;
        }
		// dừng thiết bị(theo mẫu)
		public bool StopDevice()
        {
            if (GrabbingCIC)
            {
                ...
                if (nRet != MV_OK)
                {
                    DelegateToUI.PushToListBox("Stop Grabbing Camera 2 Fail!", LogTitle.Error);
                    return false;
                }
                return true;
            }
		...
		public void CloseDevice()
        {
            if (StopDevice())
            {
                ...
                MV_CC_DestroyDevice_NET();
                DelegateToUI.PushToListBox("Device has been Camera 2 closed");
                DeviceCIC = IntPtr.Zero;
                Status.Camera2 = false;
            }
            else
            {
                if (DeviceCIC != IntPtr.Zero)
                {
                    DelegateToUI.PushToListBox("Can't close Camera 2 Device");
                }
            }
        ...
        {
            int res = MV_CC_SetEnumValue_NET("TriggerMode", (uint)MV_CAM_TRIGGER_MODE.MV_TRIGGER_MODE_ON);
            if (res != MV_OK)
            {
                DelegateToUI.PushToListBox("Set Trigger Mode Camera 2 failed!", LogTitle.Error);
            }
            res = MV_CC_SetEnumValue_NET("TriggerSource", (uint)MV_CAM_TRIGGER_SOURCE.MV_TRIGGER_SOURCE_LINE0);
            if (res != MV_OK)
            {
                DelegateToUI.PushToListBox("Set Trigger Source LINE0 Camera 2 failed!", LogTitle.Error);
            }
            res = MV_CC_SetEnumValue_NET("TriggerSource", (uint)MV_CAM_TRIGGER_SOURCE.MV_TRIGGER_SOURCE_SOFTWARE);
            //if (res != MV_OK)
            //{
            //    DelegateToUI.PushToListBox("Set Trigger Source failed!", LogTitle.Error);
            //}
        }
        // bắt đầu điều khiển (theo mẫu)
        public bool StartGrabbing()
        {
            ...
            // khởi tạo thread chờ ảnh
            ReceiveProcess = new Thread(ReceiveThreadProcess);
            ...
            if (MV_OK != nRet)
            {
                GrabbingCIC = false;
                ReceiveProcess.Join();
                DelegateToUI.PushToListBox("Start Grabbing Camera 2 Fail!", LogTitle.Error);
                CloseDevice();
                return false;
            }
            ...
            if (!GrabbingCIC)
            {
                MV_CC_SetEnumValue_NET("TriggerSource", (uint)MV_CAM_TRIGGER_SOURCE.MV_TRIGGER_SOURCE_LINE0);
            }

            return true;
        }
		// ra lệnh chụp(theo mẫu)
		public void TriggerCam()
        {
            if (Status.Camera2)
            {
                DelegateToUI.PushToListBox("Trigger CAMERA2");
                WaitingImage = true;
                int nRet = MV_CC_SetCommandValue_NET("TriggerSoftware");
                if (MV_OK != nRet)
                {
                    ShowErrorMessage(nRet);
                    DelegateToUI.PushToListBox("Trigger Camera 2 Software Fail!", LogTitle.Error);
                    ManualTrigger = false;
                }
            }
            else
            {
                MessageBox.Show("Camera 2 is not connecting!");
                ManualTrigger = false;
            }
        }
		// thiết lập trước khi điều khiển(theo mẫu)
		public int NecessaryOperBeforeGrab()
        {
            // ch:取图像宽 | en:Get Iamge Width
            MVCC_INTVALUE_EX stWidth = new MVCC_INTVALUE_EX();
            int nRet = MV_CC_GetIntValueEx_NET("Width", ref stWidth);
            if (MV_OK != nRet)
            {
                DelegateToUI.PushToListBox("Get Width Info Camera 2 Fail!", LogTitle.Error);
                return nRet;
            }
            // ch:取图像高 | en:Get Iamge Height
            MVCC_INTVALUE_EX stHeight = new MVCC_INTVALUE_EX();
            nRet = MV_CC_GetIntValueEx_NET("Height", ref stHeight);
            if (MV_OK != nRet)
            {
                DelegateToUI.PushToListBox("Get Height Info Camera 2 Fail!", LogTitle.Error);
                return nRet;
            }
            ...
            if (MV_OK != nRet)
            {
                DelegateToUI.PushToListBox("Get Pixel Format Camera 2 Fail!", LogTitle.Error);
                return nRet;
            }

            // ch:设置bitmap像素格式，申请相应大小内存 | en:Set Bitmap Pixel Format, alloc memory
            if (Convert.ToInt32(MvGvspPixelType.PixelType_Gvsp_Undefined) == stPixelFormat.nCurValue)
            {
                DelegateToUI.PushToListBox($"Unknown Pixel Camera 2 Format! ({MV_E_UNKNOW})", LogTitle.Error);
                return MV_E_UNKNOW;
            }
            else if (IsMono(stPixelFormat.nCurValue))
            {
                bmpPF = PixelFormat.Format8bppIndexed;

                if (IntPtr.Zero != ConvertBuffer)
                {
                    Marshal.Release(ConvertBuffer);
                    ConvertBuffer = IntPtr.Zero;
                }

                // Mono8为单通道
                BufferLength = Convert.ToUInt32(stWidth.nCurValue * stHeight.nCurValue);
                ConvertBuffer = Marshal.AllocHGlobal(Convert.ToInt32(BufferLength));
                if (IntPtr.Zero == ConvertBuffer)
                {
                    DelegateToUI.PushToListBox($"Malloc Memory Camera 2 Fail! ({MV_E_RESOURCE})", LogTitle.Error);
                    return MV_E_RESOURCE;
                }
            }
            else
            {
                bmpPF = PixelFormat.Format24bppRgb;
                ...
                // RGB为三通道
                BufferLength = Convert.ToUInt32(3 * stWidth.nCurValue * stHeight.nCurValue);
                ConvertBuffer = Marshal.AllocHGlobal(Convert.ToInt32(BufferLength));
                if (IntPtr.Zero == ConvertBuffer)
                {
                    DelegateToUI.PushToListBox($"Malloc Memory Camera 2 Fail! ({MV_E_RESOURCE})", LogTitle.Error);
            ...
            bmp = new Bitmap(Convert.ToInt32(stWidth.nCurValue), Convert.ToInt32(stHeight.nCurValue), bmpPF);

            // ch:Mono8格式，设置为标准调色板 | en:Set Standard Palette in Mono8 Format
            if (PixelFormat.Format8bppIndexed == bmpPF)
            {
        ...
        // ?
        private static bool IsMono(UInt32 enPixelType)
        {
            switch (enPixelType)
            {
                case (uint)MvGvspPixelType.PixelType_Gvsp_Mono1p:
                case (uint)MvGvspPixelType.PixelType_Gvsp_Mono2p:
                case (uint)MvGvspPixelType.PixelType_Gvsp_Mono4p:
                case (uint)MvGvspPixelType.PixelType_Gvsp_Mono8:
                case (uint)MvGvspPixelType.PixelType_Gvsp_Mono8_Signed:
                case (uint)MvGvspPixelType.PixelType_Gvsp_Mono10:
                case (uint)MvGvspPixelType.PixelType_Gvsp_Mono10_Packed:
                case (uint)MvGvspPixelType.PixelType_Gvsp_Mono12:
                case (uint)MvGvspPixelType.PixelType_Gvsp_Mono12_Packed:
                case (uint)MvGvspPixelType.PixelType_Gvsp_Mono14:
                case (uint)MvGvspPixelType.PixelType_Gvsp_Mono16:
                    return true;
                default:
                    return false;
            }
        }
		// thread chờ xử lý ảnh(theo mẫu)
		public void ReceiveThreadProcess()
        {
            ...
            while (GrabbingCIC)
            {
                nRet = MV_CC_GetImageBuffer_NET(ref stFrameInfo, 1000);
                if (nRet == MV_OK)
                {
                    lock (BufForDriverLock)
                    {
                        ...
                        stConvertInfo.pDstBuffer = ConvertBuffer;
                        stConvertInfo.nDstBufferSize = BufferLength;
                        ...
                        BitmapData bitmapData = bmp.LockBits(new Rectangle(0, 0, stConvertInfo.nWidth, stConvertInfo.nHeight), ImageLockMode.ReadWrite, bmp.PixelFormat);
                        CopyMemory(bitmapData.Scan0, stConvertInfo.pDstBuffer, (UInt32)(bitmapData.Stride * bmp.Height));
                        bmp.UnlockBits(bitmapData);
                    }

                    //stDisplayInfo.hWnd = FormMain.PictureBoxHandle;
                    //stDisplayInfo.pData = stFrameInfo.pBufAddr;
                    //stDisplayInfo.nDataLen = stFrameInfo.stFrameInfo.nFrameLen;
                    //stDisplayInfo.nWidth = stFrameInfo.stFrameInfo.nWidth;
                    //stDisplayInfo.nHeight = stFrameInfo.stFrameInfo.nHeight;
                    //stDisplayInfo.enPixelType = stFrameInfo.stFrameInfo.enPixelType;
                    // copy ảnh sang bit map mới
                    TrueBitmap = new Bitmap(bmp);
                    WaitingImage = false;
                    ErrorTake = false;
                    // chụp ảnh manual và lưu
                    if (ManualTrigger)
                    {
                        ManualTrigger = false;
                        //TrueBitmap = Processimage.DrawToImageNoBarcode(TrueBitmap);
                        Processimage.SaveImageManual(new Bitmap(TrueBitmap));
                        DelegateToUI.UIUpdatePicture(TrueBitmap);
                    }
                    else
                    {
                        
                    }
                    // giải phóng dữ liệu buffer
                    MV_CC_FreeImageBuffer_NET(ref stFrameInfo);
                }
                else if (nRet == MV_E_NETER)
                {
                    ShowErrorMessage(nRet);
                    ErrorTake = true;
                    ManualTrigger = false;
        ...
        public void GetCameraParameter()
        {
            ...
            if (MV_OK == nRet)
            {
                //Text = stParam.fCurValue.ToString("F1");
            }
        ...
        // set thông số cam
        public bool SetCameraParameter(float Exposure, float Gain)
        {
            MV_CC_SetEnumValue_NET("ExposureAuto", 0);
            int nRet = MV_CC_SetFloatValue_NET("ExposureTime", Exposure);
            if (nRet != MV_OK)
            {
                ShowErrorMessage(nRet);
                return false;
            ...
            MV_CC_SetEnumValue_NET("GainAuto", 0);
            nRet = MV_CC_SetFloatValue_NET("Gain", Gain);
            if (nRet != MV_OK)
            {
                ShowErrorMessage(nRet);
                return false;
        ...
        // hiển thị lỗi
        public void ShowErrorMessage(int nErrorNum)
        {
            string errorMsg = "";
            ...
            if (nErrorNum == MV_E_NETER)
            {
                Status.Camera2 = false;
            }
            Warning.Show(errorMsg+"|C2");
        }
