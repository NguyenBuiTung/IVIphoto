...
namespace TakeimgIVI.Camera
{
    ...
    public class CameraCIC
    {
        ...
        {
            if (!Status.PingHost(Constants.Camera_IP1))
            {
        ...
        public void GetDeviceInfo()
        {
            ...
            for (int i = 0; i < DeviceList.nDeviceNum; i++)
            {
                ...
                if (device.nTLayerType == MV_GIGE_DEVICE)
                {
                    ...
                    if (GetDeviceIp(gigeInfo.nCurrentIp) == Constants.Camera_IP1)
                    {
                        Constants.Camera_IP1 = GetDeviceIp(gigeInfo.nCurrentIp);
                        INI.WriteIni(Constants.Common, "Camera", "IP1", Constants.Camera_IP1);
                        DeviceInfo = device;
                        IP = Constants.Camera_IP1;
            ...
            {
                DelegateToUI.PushToListBox("Get Device Camera 1 successfully!");
            }
            else
            {
                DelegateToUI.PushToListBox("Fail to get Camera 1 Device", LogTitle.Error);
            }
        }
		...
		public bool OpenDevice()
        {
            ...
            if (MV_OK != Status)
            {
                DelegateToUI.PushToListBox("Create device Camera 1 fail!", LogTitle.Error);
                return false;
            }
            ...
            if (MV_OK != Status)
            {
                ...
                DelegateToUI.PushToListBox("Device open Camera 1 fail!", LogTitle.Error);
                return false;
            }
            ...
            if (nPacketSize > 0)
            {
                ...
                if (Status != MV_OK)
                {
                    DelegateToUI.PushToListBox("Set Packet Size Camera 1 failed!", LogTitle.Error);
                }
            }
            else
            {
                DelegateToUI.PushToListBox("Get Packet Size Camera 1 failed!", LogTitle.Error);
            }
		...
		public bool StopDevice()
        {
            if (GrabbingCIC)
            {
                ...
                if (nRet != MV_OK)
                {
                    DelegateToUI.PushToListBox("Stop Grabbing Camera 1 Fail!", LogTitle.Error);
                    return false;
                }
                return true;
            }
		...
		public void CloseDevice()
        {
            if (StopDevice())
            {
                ...
                MV_CC_DestroyDevice_NET();
                DelegateToUI.PushToListBox("Device has been Camera 1 closed");
                DeviceCIC = IntPtr.Zero;
                Status.Camera1 = false;
            }
            else
            {
                if (DeviceCIC != IntPtr.Zero)
                {
                    DelegateToUI.PushToListBox("Can't close Camera 1 Device");
                }
            }
        ...
        public void SetTriggerMode()
        {
            ...
            if (res != MV_OK)
            {
                DelegateToUI.PushToListBox("Set Trigger Mode Camera 1 failed!", LogTitle.Error);
            }
            res = MV_CC_SetEnumValue_NET("TriggerSource", (uint)MV_CAM_TRIGGER_SOURCE.MV_TRIGGER_SOURCE_LINE0);
            if (res != MV_OK)
            {
                DelegateToUI.PushToListBox("Set Trigger Source LINE0 Camera 1 failed!", LogTitle.Error);
            }
        ...
        public bool StartGrabbing()
        {
            ...
            if (MV_OK != nRet)
            {
                GrabbingCIC = false;
                ReceiveProcess.Join();
                DelegateToUI.PushToListBox("Start Grabbing Camera 1 Fail!", LogTitle.Error);
                CloseDevice();
                return false;
            }
		...
		public void TriggerCam()
        {
            if (Status.Camera1)
            {
                DelegateToUI.PushToListBox("Trigger CAMERA1");
                WaitingImage = true;
                int nRet = MV_CC_SetCommandValue_NET("TriggerSoftware");
                if (MV_OK != nRet)
                {
                    ShowErrorMessage(nRet);
                    DelegateToUI.PushToListBox("Trigger Software Camera 1 Fail!", LogTitle.Error);
                    ManualTrigger = false;
                }
            }
            else
            {
                MessageBox.Show("Camera 1 is not connecting!");
                ManualTrigger = false;
            }
        }
		...
		public int NecessaryOperBeforeGrab()
        {
            ...
            if (MV_OK != nRet)
            {
                DelegateToUI.PushToListBox("Get Width Info Camera 1 Fail!", LogTitle.Error);
                return nRet;
            }
            ...
            if (MV_OK != nRet)
            {
                DelegateToUI.PushToListBox("Get Height Info Camera 1 Fail!", LogTitle.Error);
                return nRet;
            }
            ...
            if (MV_OK != nRet)
            {
                DelegateToUI.PushToListBox("Get Pixel Format Camera 1 Fail!", LogTitle.Error);
                return nRet;
            }
            ...
            {
                DelegateToUI.PushToListBox($"Unknown Pixel Camera 1 Format! ({MV_E_UNKNOW})", LogTitle.Error);
                return MV_E_UNKNOW;
            }
            else if (IsMono(stPixelFormat.nCurValue))
            {
                ...
                if (IntPtr.Zero == ConvertBuffer)
                {
                    DelegateToUI.PushToListBox($"Malloc Memory Camera 1 Fail! ({MV_E_RESOURCE})", LogTitle.Error);
            ...
            while (GrabbingCIC)
            {
                ...
                if (nRet == MV_OK)
                {
                    ...
                    if (ManualTrigger)
                    {
                        ...
                        //MV_CC_DisplayOneFrame_NET(ref stDisplayInfo);
                    }
                    else
                    {
